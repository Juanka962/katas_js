...
files created later with the same name as a file created earlier should have an additional (k) suffix in their names, where k is the smallest positive integer 
...
function solution(names) {
   ...
   for (let i = 0; i < names.length; i++) {
     let cont = 1;
     let nameUnique = names[i];
 
     while (namesFilter.has(nameUnique)) {
       nameUnique = `${names[i]}(${cont})`;
       cont++;
     }
 
     namesFilter.add(nameUnique);
     arr.push(nameUnique);
   }
